...
const NavButton: React.FC<{ item: NavItem }> = ({ item }) => (
    <div className="flex flex-col items-center space-y-2 group">
        <button className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center shadow-md hover:shadow-lg hover:bg-teal-400 text-teal-500 hover:text-white transition-all duration-300 transform hover:-translate-y-1">
            <item.icon />
        </button>
        <span className="text-sm font-medium text-gray-600 group-hover:text-teal-500 transition-colors duration-300">{item.label}</span>
    </div>
);
